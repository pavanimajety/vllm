...
"""TRTLLM GQA builder: reorder threshold under DCP with spec decode."""
...
if not current_platform.is_cuda():
    pytest.skip("TRTLLM backend requires a CUDA platform.", allow_module_level=True)
...
def test_trtllm_gqa_dcp_spec_decode_clamps_reorder_threshold(monkeypatch):
    ...
    monkeypatch.setattr(
        trtllm_backend, "can_use_trtllm_attention", lambda *args, **kwargs: True
    )
    monkeypatch.setattr(
        TRTLLMMetadataBuilder,
        "_get_trtllm_api_decode_kernel",
        staticmethod(lambda: TrtllmDecodeAPIKernel.TRTLLM_GEN),
    )
    monkeypatch.setattr(
        trtllm_backend,
    ...
    assert builder.trtllm_api_decode_kernel == TrtllmDecodeAPIKernel.TRTLLM_GEN
